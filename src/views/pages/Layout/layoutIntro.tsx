...
export function LayoutRootIntro({ children }: React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement>) {
  ...
  return (
    <section className="bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(119, 198, 181, 0.541),rgba(255,255,255,0))] absolute top-0 z-[-2] h-screen w-screen">
      ...
      <div className="mx-6 flex h-screen flex-col pt-4">
        <div className="flex items-center justify-between">
          <Button size="icon" className='bg-transparent text-primary hover:bg-transparent dark:text-primary' onClick={() => navigate(-1)}>
            <CircleChevronLeft />
          </Button>

          <Button size="sm" className="font-display px-0 text-primary bg-transparent hover:bg-transparent ">
            <Link to={routes.home}>Pular</Link>
          </Button>
        </div>
